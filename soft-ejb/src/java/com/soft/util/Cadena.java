...
public class Cadena {
    ...
    public static boolean isCelularValid(String numCel) {
        if (!Numero.isNumeric(numCel)) {
            return false;
        }
        if (new Long(numCel).longValue() <= 0) {
            return false;
        }
        if (numCel.trim().length() < 8) {
            return false;
        }
    ...
    public static void main(String args[]) {
        ...
        System.out.println(new Cadena().reemplazaEspacios("Carrrera 15 # 85 - 29", "&"));
        ...
        System.out.println("isAl: " + isAlphanumeric("FIY846"));

        System.out.println("Money:" + formatMoney(1500000));
